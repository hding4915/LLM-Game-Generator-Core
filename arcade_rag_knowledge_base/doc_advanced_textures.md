Source: Arcade 2.6.17 doc/advanced/textures.rst
...
The :py`arcade.Texture` type is how arcade normally interacts with
...
OpenGL API: :py`arcade.gl.Texture`. This represents an
...
The above also applies when using :py`arcade.load_texture` or other
texture loading functions.

Arcade's texture cache can be cleared using :py`arcade.cleanup_texture_cache`.
