Source: doc/programming_guide/camera.rst
...
- Objects which modify the view and perspective matrices are called "Projectors"
  ...
  - :py`~arcade.camera.Projector` is a :py`Protocol` used internally by arcade
  - :py`~arcade.camera.Projector.use` sets the internal projection and view matrices used by Arcade and Pyglet
  - :py`~arcade.camera.Projector.activate` is the same as use, but works within a context manager using the ``with`` syntax
  - :py`~arcade.camera.Projector.unproject` provides a way to find the world position of any pixel position on screen.
  - :py`~arcade.camera.Projector.project` provides a way to find the screen position of any position in the world.

- There are multiple data types which provide the information required to make view and projection matrices

  - :py`~arcade.camera.CameraData` holds the position, forward, and up vectors along with a zoom value used to create the view matrix
  - :py`~arcade.camera.OrthographicProjectionData` holds the left, right, bottom, top, near, far values needed to create a orthographic projection matrix
  - :py`~arcade.camera.PerspectiveProjectionData` holds the aspect ratio, field of view, near and far needed to create a perspective projection matrix.

- There are three primary `Projectors` in `arcade.camera`

  - :py`~arcade.Camera2D` is locked to the x-y plane and is perfect for most use cases within arcade.
  - :py`~arcade.camera.OrthographicProjector` can be freely positioned in 3D space, and the scale of objects does not depend on the distance from the projector.
  - :py`~arcade.camera.PerspectiveProjector` can be freely positioned in 3D space, and objects look smaller the further from the camera they are.
