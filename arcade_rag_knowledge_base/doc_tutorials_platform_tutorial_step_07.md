Source: doc/tutorials/platform_tutorial/step_07.rst
...
To achieve this, we can use a Camera. Since we are making a 2D game, :py`~arcade.Camera2D` will
be easiest.
...
Since we're drawing to the entire screen, we can use :py`~arcade.Camera2D`'s default settings.
