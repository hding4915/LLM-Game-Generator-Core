Source: doc/programming_guide/opengl_notes.rst
...
:py`~arcade.SpriteList` can for example be built before window creation
and will be initialized internally in the first draw call.

:py`~arcade.DefaultTextureAtlas` on the other hand cannot
be crated before the window is created, but :py`~arcade.Texture`
