Source: doc/programming_guide/event_loop.rst
...
All three methods are exposed to be overridden in :py`arcade.Window`
and :py`arcade.View`. For advanced use cases it is possible to add own
handler via :py`arcade.Window.push_handlers`.
...
:py`arcade.Window` with the ``draw_rate`` argument.
...
the ``update_rate`` argument when initializing your :py`arcade.Window`.
...
called with the ``fixed_rate`` argument when initializing your :py`arcade.Window`.
...
clocks, possibly ticking at a different rate, an :py`arcade.clock.Clock`
can be created on the fly.

:py`~arcade.clock.FixedClock`
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
Like the regular clock you may make a new :py`~arcade.clock.FixedClock` at any time,
but ensure they have a sibling.

Up Coming
^^^^^^^^^

In future version of arcade :py`~arcade.clock.Clock` will be updated to allow for sub clocks. 
...
slowing enemies or excluding UI elements. To gain access to a draft :py`~arcade.clock.Clock`
...
:py`arcade.Window`. The second method is to slow-down time temporarily. By changing the
