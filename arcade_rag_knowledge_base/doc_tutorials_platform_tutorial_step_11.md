Source: doc/tutorials/platform_tutorial/step_11.rst
...
Arcade provides a better way to handle this, with the `arcade.Scene` class. This class will hold all of
...
Let's analyze what happens when we do `arcade.Scene.add_sprite`. The first parameter to it is a String,
...
are familiar with Python dictionaries, the `arcade.Scene` class can be interacted with in
a very similar way. You can get any specific SpriteList within the scene by passing the name in
brackets to the scene.

We need to also change our `arcade.check_for_collision_with_list` function in ``on_update`` that
