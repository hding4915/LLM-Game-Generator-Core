Source: Arcade 2.6.17 doc/advanced/headless.rst
...
In headless mode the arcade :py`~arcade.Window` will extend
...
created through the :py`~arcade.ArcadeContext` if needed.
    ...
    caches :py`arcade.Texture` objects. See
    :py`~arcade.cleanup_texture_cache`.
    This might also
...
There are two recommended approaches: simple_headless_approach
and extend_arcade_window_headless.
...
You can also split your code into :py`arcade.View` classes
