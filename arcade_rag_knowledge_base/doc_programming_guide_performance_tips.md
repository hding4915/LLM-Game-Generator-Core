Source: doc/programming_guide/performance_tips.rst
...
#. Collision detection <collision_detection_performance>
#. Drawing <drawing_performance>
#. Loading-related issues <loading_performance>
...
Imagine you have 50,000 :py`~arcade.Sprite` instances:
...
of both the player and the ground :py`~arcade.Sprite` objects are squares,
...
#. The built-in collision_detection_performance_hashing
#. The Pymunk physics engine integrations <collision_detection_performance_pymunk>
...
against a :py`~arcade.SpriteList` of
**non-moving** sprites:
...
uses a **hash map** (:py`dict`) of grid square coordinates to lists
of :py`~arcade.Sprite` objects in each square.

How does this help us? We may need as few as zero hitbox checks to collide
a given sprite against a :py`~arcade.SpriteList`. Yes,
**zero**:

   :alt: A blue bird is alone in its own grid square.

#. The sand-colored ground consists of sprites in a
   :py`~arcade.SpriteList` with spatial hashing enabled
...
* how many :py`~arcade.Sprite` objects are in any given square
* the size of each :py`~arcade.Sprite` passed
...
:py`~arcade.SpriteList` is before anything else,
especially before gameplay.

The simplest way is passing ``use_spatial_hash=True`` when creating
and storing the list inside a :py`~arcade.Window` or
:py`~arcade.View`:
...
For a runnable example of this, please see camera_platform. Additional
examples are linked below in collision_performance_spatial_hashing_examples.
...
* platformer_part_three
* platformer_part_twelve

For detailed API notes, please see:

* :py`arcade.SpriteList`
* :py:meth:`arcade.SpriteList.enable_spatial_hashing`
* :py`arcade.sprite_list.spatial_hash.SpatialHash`
* :py`arcade.physics_engines.PhysicsEngineSimple`
* :py`arcade.physics_engines.PhysicsEnginePlatformer`
...
The hash map is a Python :py`dict` mapping :py`tuple`
coordinate pairs to :py`list` instances.
...
* The example-code-pymunk Example
* Arcade's pymunk_platformer_tutorial tutorial
* :py`arcade.pymunk_physics_engine.PymunkPhysicsEngine`
...
#. Use Arcade's non-modifiable shapes with `arcade.shape_list.ShapeElementList`
...
Arcade's `arcade.SpriteList` is the only way to draw a :py`~arcade.Sprite`.

This is because all drawing with a graphics card is batched drawing.
The :py`~arcade.SpriteList` handles batching for you. As a result,
you can draw thousands of moving sprites with any extra effort on your
part.

An Option for Advanced Users
^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Advanced users may want to try pyglet's :py`pyglet.sprite.Sprite`.

Instead of Arcade's :py`~arcade.SpriteList`, pyglet sprites use a
mix of the following classes:

* :py`pyglet.graphics.Batch`
* :py`pyglet.graphics.Group`
...
#. Use :py`arcade.Text` instead
#. (Optional) Pass a pyglet :py`~pyglet.graphics.Batch` object at creation
...
If you are not familiar, you may want to read sound-loading-modes
before proceeding.
...
streaming <sound-loading-modes-streaming> from compressed files
instead.
...
#. The :py`~arcade.sound.Sound` will have at most one playback at a time.
...
* sound-intermediate-playback-change-aspects-ongoing
* The :py`pyglet.media.StreamingSource` class used to implement
  streaming
