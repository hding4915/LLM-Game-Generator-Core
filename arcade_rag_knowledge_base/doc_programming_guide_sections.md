Source: doc/programming_guide/sections.rst
...
A `~arcade.Section` is a way to divide a `~arcade.View` space into smaller parts,
each one will then receive events redirected depending on configuration and the
space of the view occupied.
Sections can isolate code that otherwise goes packed together in a `~arcade.View`
. This way the code remains exactly where it belongs and not mixed together with
code from other parts of the program.

By configuring a `~arcade.Section` you can capture some events or for example only
...
the `~arcade.SectionManager` is not created if you don't add sections.

**Key features of Sections:**

 - Divide the screen into logical components (Sections).
 - Event dispatching: a `~arcade.Section` will capture mouse events based on the space occupied from the view. Also keyboard events will be captured based on configuration.
 - Prevent dispatching: a `~arcade.Section` can be configured to prevent dispatching events captured or let events flow to other sections underneath.
 - Event capturing order: based on a `~arcade.Section` insertion order you can configure the order in which sections will capture events.
 - Draw order: you can configure the order in which sections are drawn (sections can overlap!).
 - `~arcade.Section` "enable" property to show or hide sections. You can toggle that.
...
To work with sections you first need to have a `~arcade.View`. Sections depend on
Views and are handled by a special `~arcade.SectionManager` inside the
`~arcade.View`. The `~arcade.SectionManager` will handle all the events and
dispatch them to the sections. The `~arcade.SectionManager` will also handle the
drawing order of the sections.

You will have to enable and disable the `~arcade.SectionManager` in the `~arcade.View`
``on_show_view`` and ``on_hide_view`` methods.

To create a `~arcade.Section` start by inheriting from :py`~arcade.Section`.

Based on the `~arcade.Section` configuration your section will start receiving
events from the View `~arcade.SectionManager`. A `~arcade.Section` has all the
events a `~arcade.View` has like ``on_draw``, ``on_update``, ``on_mouse_press``,
etc.

On instantiation define the positional arguments (left, bottom, width, height)
of the section. These are very important properties of a `~arcade.Section`: as they
define the event capture rectangular area.


Properties of a `~arcade.Section`:

**position: (left, bottom, width, height)**:
    This are mandatory arguments that you need to provide when instantiating a
    `~arcade.Section`. This is very important as this rectangular positioning
...
**name:**
    A `~arcade.Section` can optionally get a name so it will be easier to
    debug and identify what Section is doing what. When logging for example
    is very nice to log the `~arcade.Section` name at the beginning so you
    have a reference from where the log was generated.

**accept_keyboard_keys:**
    This allows to tell if a `~arcade.Section` can receive keyboard events
    (accept_keyboard_keys=False) or to tell which keyboard keys are captured
    in this `~arcade.Section` (accept_keyboard_keys={arade.key.UP, arcade.key.DOWN})

**accept_mouse_events:**
    This allows to tell if a `~arcade.Section` can receive mouse events or which
    mouse events are accepted.
    For example: accept_mouse_events={'on_mouse_move'} means only mouse move events
    will be captured.

**prevent_dispatch:**
    This tells a `~arcade.Section` if it should prevent the dispatching of certain
    events to other sections down event capture stream. By default a `~arcade.Section`
...
**prevent_dispatch_view:**
    This allows to tell a `~arcade.Section` if events (and what events) should
    not be dispatched to the underlying `~arcade.View`.
    This is handy if you want to do some action in the `~arcade.View` code whether
    or not the event was handled by another `~arcade.Section`. By default a
    `~arcade.Section` will prevent dispatching all handled events to the `~arcade.View`.
    Note that passing ``prevent_dispatch=None`` (the default) is the same as passing
...
**enabled:**
    ...
    `~arcade.Section` should be enabled or not. If a `~arcade.Section` is not
...
**modal:**
    This tells the `~arcade.SectionManager` that this `~arcade.Section` is modal.
    This means that the `~arcade.Section` will capture all events first and not
    deliver any events to the underlying sections or view. Also, It will draw
    last (on top of other ``on_draw`` calls). When enabled a modal `~arcade.Section`
    will prevent all other sections from receive ``on_update`` events.

**draw_order:**
    This allows to define the draw order this `~arcade.Section` will have.
    ...
    `~arcade.Section` to be drawn on top of another.
    ...
    which is defined by the insertion order in the `~arcade.SectionManager`.

Other handy `~arcade.Section` properties:

- block_updates: if True this section will not have the ``on_update`` method called.
- camera: this is meant to hold a :py`~arcade.Camera2D` but it is None by default. The SectionManager will trigger the use of the camera when is needed automatically.

Handy `~arcade.Section`: methods:

- overlaps_with: this will tell if another `~arcade.Section` overlaps with this one.
...
* ``on_mouse_enter`` and ``on_mouse_leave``:
    ...
    view and when the `~arcade.SectionManager` detects by mouse motion (or dragging)
...
* ``on_show_section`` and ``on_hide_section``:
    ...
    - When the section is added or removed from the `~arcade.SectionManager` and the `~arcade.View` is currently being shown
...
Behind the scenes, when sections are added to the `~arcade.View` the
`~arcade.SectionManager` is what will handle all events instead of
the `~arcade.View` itself.

Usually you won't need to work with the `~arcade.SectionManager`, but there are
...
the `~arcade.SectionManager`. Also you have a ``remove_section`` and a
``clear_sections`` method.

You can ``enable`` or  ``disable`` the `~arcade.SectionManager` to completely
enable or disable all sections at once.

There are some other functionality exposed from the `~arcade.SectionManager` like
``get_section_by_name`` that can also be useful. Check the api to know about those.

Also there are three attributes that can be configured in the `~arcade.SectionManager`
that are useful and important sometimes.

By default, ``on_draw``, ``on_update`` and ``on_resize`` are events that will always
be triggered in the `~arcade.View` before any section has triggered them.
...
- True (default) to trigger that event in the `~arcade.View` before the sections.
- False so it's triggered in the `~arcade.View` after sections corresponding methods.
- None to not trigger that event in the `~arcade.View` at all.
