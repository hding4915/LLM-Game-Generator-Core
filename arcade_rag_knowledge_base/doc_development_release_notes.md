Source: Arcade 2.6.17 doc/development/release_notes.rst
...
* Various Improvements
  ...
  * Allow specifying hit box parameters in :py`~arcade.load_textures` and
    :py`~arcade.load_spritesheet`
  * :py`~arcade.Camera` should no longer apply zoom on the z axis
  ...
  * Added :py`~arcade.get_angle_radians`
...
* New Features
  ...
  * Arcade can now run in headless mode on linux servers opening
    ...
    See headless for more information.
...
* Bugfixes
  ...
  * Fixed an issue causing :py`arcade.draw_circle_filled` and
    :py`arcade.draw_circle_outline` to always render with 3 segments
    on some iGPUs.
...
* Various Improvements
  ...
  * :py`arcade.load_font` now supports resource handles
  * :py`~arcade.PhysicsEngineSimple` can now take an iterable of wall spritelists
...
* Docs
  ...
  * Fixed typo in :py`~arcade.Sprite.append_texture` docstring(`#1126 <https://github.com/pythonarcade/arcade/pull/1126>`_)
...
* OpenGL
  ...
  * :py`arcade.gl.Geometry` now supports transforming to multiple buffers.
...
* General:
  ...
  * Bugfix: :py`~arcade.check_for_collision_with_list` selected
  ...
  * :py`arcade.Text` now has a ``left``, ``right`` ``top``
    and ``bottom`` attribute for getting the pixel locations
    of the content borders.
  * Added performance warning for :py`arcade.draw_text`.
    Using :py`arcade.Text` is a lot faster. We have
...
* Documentation
  ...
  * Work on shader_toy_tutorial_glow.
  * Docstring improvements throughout the code base
  * Many examples are cleaned up

* OpenGL

  * :py`arcade.gl.Buffer` is guaranteed to contain
    zero byte values on creation.
  * Expose :py`~arcade.gl.context.Limits` in :py:attr:`arcade.gl.Context.info`
  ...
  * :py`arcade.gl.ComputeShader` is now part of the gl module
  * :py`arcade.gl.ComputeShader` was added to docs
  * Expose and document :py`arcade.gl.context.ContextStats`
...
* Sections - Add support to divide window into sections.
  ...
  * Add examples on how to use: section_examples
...
* Misc:
  ...
  * `#1095 <https://github.com/pythonarcade/arcade/issues/1095>`_,
    Improvements to `arcade.Text` and its documentation.
  ...
  * :py`arcade.draw_text` and :py`arcade.Text` objects
    now accepts any python object as text and converts it into
    a string internally if needed.
  * :py`~arcade.SpriteList` now exposes several new members
...
* Sprites
  ...
  * :py`~arcade.SpriteList` now supports setting a global color and alpha value.
  ...
  * The :py`~arcade.Sprite` initializer now also accepts ``None`` value for ``hit_box_algorithm``
...
* ``arcade.gl``
  ...
  * The :py`~arcade.gl.Query` object now allows for selecting what specific queries should be performed
...
* ``arcade.gui``
  ...
  * :py`~arcade.gui.UIBoxLayout` supports now align in constructor (changing later requires a `UIBoxLayout.trigger_full_render()`).
  * :py`~arcade.gui.UIBoxLayout` supports now space_between in constructor.
  * :py`~arcade.gui.UIManager` fix #1067, consume press and release mouse events
...
* `~arcade.tilemap.TileMap` changes:
...
* `~arcade.Sprite` changes:
  ...
  * :py`~arcade.SpriteCircle` and :py`SpriteSolidColor` textures are now cached internally
    for better performance.

* `~arcade.PhysicsEnginePlatformer` Optimization:
...
* `~arcade.TileMap` additions and fixes:
...
* UI
  ...
  * `~arcade.UIInputText` now supports both RGB and RGBA text color
...
* Text
  ...
  * The :py`~arcade.Text` object is now usable and is preferred over
    :py`arcade.draw_text` in many cases for performance reasons.
  * Text related functions should now have better documentation

* Misc:

  * Added support to the `~arcade.View` class for :meth:`~arcade.View.on_resize`
  ...
  * New utility function `~arcade.color_from_hex_string` that will turn a hex string into a color.
  ...
  * Addition of `~arcade.PymunkException` class for throwing Pymunk errors in the
    Pymunk physics engine.
  * The `~arcade.check_for_collision_with_lists` function will now accept any Iterable(List, Tuple, Set, etc) containing SpriteLists.
...
* `Tiled Map Editor <https://www.mapeditor.org/>`_ support has been overhauled.
  ...
  * See platformer_tutorial for a how-to, Tiled usage starts at Chapter 9.
...
* Texture atlases have been introduced, texture management has been improved.
  ...
  * The texture atlas portion of a sprite can be drawn to, and quickly updated on the GPU side.
    ...
    * To demonstrate, there is a new minimap example that creates a sprite that has a dynamic minimap
      projected onto it.
...
* Shadertoy support.
  ...
  * See shader_toy_tutorial_glow and `Asteroids <https://github.com/pythonarcade/asteroids>`_.
...
* Reworked GUI
  ...
  * More documentation (gui_concepts)
  * Available Elements:

    * `~arcade.gui.UIWidget`:

      * `~arcade.gui.UIFlatButton` - 2D flat button for simple interactions (hover, press, release, click)
      * `~arcade.gui.UITextureButton` - textured button (use :meth:`arcade.load_texture()`) for simple interactions (hover, press, release, click)
      * `~arcade.gui.UILabel` - Simple text, supports multiline
      * `~arcade.gui.UIInputText` - field to accept user text input
      * `~arcade.gui.UITextArea` - Multiline scrollable text widget.
      * `~arcade.gui.UISpriteWidget` - Embeds a Sprite within the GUI tree

    * `~arcade.gui.UILayout`:

        * `~arcade.gui.UIBoxLayout` - Places widgets next to each other (vertical or horizontal)

    * `~arcade.gui.UIWrapper`:

        * `~arcade.gui.UIPadding` - Add space around a widget
        * `~arcade.gui.UIBorder` - Add border around a widget
        * `~arcade.gui.UIAnchorWidget` - Used to position UIWidgets relative on screen

    * Constructs

        * `~arcade.gui.UIMessageBox` - Popup box with a message text and a few buttons.

    * Mixins

        * `~arcade.gui.UIDraggableMixin` - Makes a widget draggable.
        * `~arcade.gui.UIMouseFilterMixin` - Catches mouse events that occure within the widget boundaries.
        * `~arcade.gui.UIWindowLikeMixin` - Combination of `~arcade.gui.UIDraggableMixin` and `~arcade.gui.UIMouseFilterMixin`.
...
* Scene Manager.
  ...
  * There is now a new `arcade.Scene` class that can be used to manage SpriteLists and their draw order.
    This can be used in place of having to draw multiple spritelists in your draw function. 
  * Contains special integration with `arcade.TileMap` using `arcade.Scene.from_tilemap` which will
    automatically create an entire scene from a loaded tilemap in the proper draw order.
  * See platformer_tutorial for an introduction to this concept, and it is used heavily throughout that tutorial.
...
* Camera support
  ...
  * Several other examples and tutorials make use of this class, like platformer_tutorial.

* Add a set of functions to track performance statistics. See perf_info_api.
* Added the class `arcade.PerfGraph`, a subclass of Sprite that will graph FPS or time to process a dispatch-able
...
* Documentation
  ...
  * The quick_index has been reorganized to be easier to find commands, and
    the individual API documentation pages have been broken into parts, so it isn't one large monolithic page.
  * New tutorial for raycasting_tutorial.

       :width: 50%

  * New tutorial for shader_toy_tutorial_glow.
...
* Better PyInstaller Support
  ...
  See bundle_into_redistributable for an example of how to use it.
...
* Add conway_alpha example showing how to use alpha to control display
  of sprites in a grid.
* Improve documentation around sound API.
* Improve documentation with FPS and timing stats example.
* Improve moving platform docs a bit in platformer_tutorial tutorial.
...
* Support for defining your own frame buffers, shaders, and more
  advanced OpenGL programming. New API in Arcade Open GL.
    ...
    * Use frame buffers to create a 'glow' or 'bloom' effect: bloom_defender.
    * Use frame-buffers to support lights: light_demo.

* New support for style-able GUI elements.
* PyMunk engine for platformers. See tutorial: pymunk_platformer_tutorial.
...
* A :py`~arcade.gl.Context` and :py`arcade.ArcadeContext` object was
...
* New :py`~arcade.gl.Texture` class supporting a wide variety of formats such as 8/16/32 bit
...
* New :py`~arcade.gl.Buffer` class with methods for manipulating data such as
  simple reading/writing and copying data from other buffers. This buffer can also
  now be bound as a uniform buffer object.
* New :py`~arcade.gl.Framebuffer` wrapper class making us able to render any content into
  one more more textures. This opens up for a lot of possibilities.
* The :py`~arcade.gl.Program` has been expanded to support geometry shaders and transform feedback
...
* A higher level type :py`~arcade.gl.Geometry` was introduced to make working with
...
* A :py`~arcade.gl.Query` class was added for easy access to low level
...
* New Tutorial: pymunk_platformer_tutorial
* New Tutorial: view-tutorial
* New Tutorial: solitaire_tutorial
* New Tutorial: gpu_particle_burst
* Several new and updated examples on example-code
...
* Enhancement: Close enhancement-related github issues and reference them in the new enhancement_list.
