Source: doc/programming_guide/gui/concepts.rst


## GUI Concepts

GUI elements are represented as instances of :py`~arcade.gui.UIWidget`.
The GUI is structured like a tree; every widget can have other widgets as
children.

The root of the tree is the :py`~arcade.gui.UIManager`. The
:py`~arcade.gui.UIManager` connects the user interactions with the GUI. Read more about
UIEvent.
...
:py`~arcade.gui.UIManager` is the starting point for the GUI.

To use the GUI, you need to create a :py`~arcade.gui.UIManager` instance and
call its :py:meth:`~arcade.gui.UIManager.add` method to add widgets to the GUI.
Each :py`~arcade.View` should have its own :py`~arcade.gui.UIManager`.
(If you don't use views, you can use a single :py`~arcade.gui.UIManager` for the window.)

The :py`~arcade.gui.UIManager` does not react to any user input initially.
...
The :py`~arcade.gui.UIView` class is a subclass of :py`~arcade.View` and provides
a convenient way to use the GUI. It instantiates a :py`~arcade.gui.UIManager` which can be accessed
via the :py:attr:`~arcade.gui.UIView.ui` attribute.
It automatically enables and disables the
:py`~arcade.gui.UIManager` when the view is shown or hidden.

UIWidget
````````

The :py`~arcade.gui.UIWidget` class is the core of Arcade's GUI system.
...
A `~arcade.gui.UIWidget` has following properties.
...
``children``
    Child widgets rendered within this widget. A `~arcade.gui.UIWidget` will not
    move or resize its children; use a :py`~arcade.gui.UILayout`
    instead.
...
``size_hint_min``
    ...
    widget. These values should be taken into account by `~arcade.gui.UILayout` when
...
``size_hint_max``
    ...
    They are hints to `~arcade.gui.UILayout` instances, which may choose to use or
    ignore them.

UILayout
````````

:py`~arcade.gui.UILayout` are widgets, which reserve the right to move
or resize children. They might respect special properties of a widget like
``size_hint``, ``size_hint_min``, or ``size_hint_max``.

The :py`arcade.gui.UILayout` must only resizes a child's dimension (x or y
...
The :py`~arcade.gui.UIManager` `draw` method, will check if updates are required and
finally draws the framebuffer on screen.

Layouting and Rendering
```````````````````````

:py`~arcade.gui.UIManager` triggers layouting and rendering of the GUI before the actual frame draw (if necessary).
...
:py`arcade.gui.UIManager` triggers the layout and render process right
before the actual frame draw. This opens the possibility to adjust to multiple
changes only once.

**Example**: Executed steps within :py`~arcade.gui.UIBoxLayout`:
...
+======================================+============+================+================+
| `~arcade.gui.UIAnchorLayout`  | X          | X              | X              |
+--------------------------------------+------------+----------------+----------------+
| `~arcade.gui.UIBoxLayout`     | X          | X              | X              |
+--------------------------------------+------------+----------------+----------------+
| `~arcade.gui.UIGridLayout`    | X          | X              | X              |
+--------------------------------------+------------+----------------+----------------+
| `~arcade.gui.UIManager`       | X          | X              | X              |
+--------------------------------------+------------+----------------+----------------+
...
- :py`~arcade.gui.UIDraggableMixin` - Makes a widget draggable with the mouse.
- :py`~arcade.gui.UIMouseFilterMixin` - Captures all mouse events.
- :py`~arcade.gui.UIWindowLikeMixin` - Makes a widget behave like a window, combining draggable and mouse filter behaviour.
...
- :py`~arcade.gui.UIMessageBox` - A simple message box with a title, message and buttons.
- :py`arcade.gui.UIButtonRow` - A row of buttons.
...
:py`~arcade.gui.UILabel` component of the button.

Flat button
^^^^^^^^^^^

**Name**: :py`~arcade.gui.UIFlatButton`
...
**Name**: :py`~arcade.gui.UITextureButton`

An image button. Textures are supplied from :py`arcade.load_texture` for
...
**Name**: :py`~arcade.gui.UIInputText`

A text field allows a user to input a basic string. It uses pyglet's
:py`~pyglet.text.layout.IncrementalTextLayout` and its
:py`~pyglet.text.caret.Caret`. These are stored in ``layout`` and
``caret`` properties.
...
:py`~arcade.gui.UIManager` into `~arcade.gui.UIEvent` and passed via
:py:meth:`~pyglet.event.EventDispatcher.dispatch_event` to the
:py:meth:`~arcade.gui.UIWidget.on_event` callbacks.

Widget-specific events (such as :py`~arcade.gui.UIOnClickEvent` are
...
- :py`arcade.gui.UIEvent`. Base class for all events.
- :py`arcade.gui.UIMouseEvent`. Base class for mouse-related events.
    - :py`arcade.gui.UIMouseMovementEvent`. Mouse motion. This event
      has an additional ``pos`` property that returns a tuple of the x and y
      coordinates.
    - :py`~arcade.gui.UIMousePressEvent`. Mouse button pressed.
    - :py`~arcade.gui.UIMouseDragEvent`. Mouse pressed and moved (drag).
    - :py`~arcade.gui.UIMouseReleaseEvent`. Mouse button release.
    - :py`~arcade.gui.UIMouseScrollEvent`. Mouse scroll.
- :py`~arcade.gui.UITextEvent`. Text input from user. This is only used
  for text fields and is the text as a string that was inputed.
- :py`~arcade.gui.UITextMotionEvent`. Text motion events. This includes
...
- :py`~arcade.gui.UITextMotionSelectEvent`. Text motion events for
...
- :py`~arcade.gui.UIOnUpdateEvent`. This is a callback to the arcade
  :py`~arcade.Window.on_update` method.
...
- :py`~arcade.gui.UIOnClickEvent`. Click event of
  :py`~arcade.gui.UIInteractiveWidget` class. This is triggered on
  widget press.
- :py`~arcade.gui.UIOnChangeEvent`. A value of a
  :py`~arcade.gui.UIWidget` has changed.
- :py`~arcade.gui.UIOnActionEvent`. An action results from interaction
  with the :py`~arcade.gui.UIWidget` (mostly used in constructs)
...
dispatched from a specific :py`~arcade.gui.UIWidget` like an ``on_click``
of a button.
...
:py`~arcade.gui.UIWidget.on_event` method. In that case, refer to
existing widgets as an example.

Pyglet window events
````````````````````

Pyglet window events are received by :py`~arcade.gui.UIManager`.

You can dispatch them via::

    UIWidget.dispatch_event("on_event", UIEvent(...))

Window events are wrapped into subclasses of :py`~arcade.gui.UIEvent`.

Pyglet event dispatcher - UIWidget
``````````````````````````````````

Widgets implement pyglet's :py`~pyglet.event.EventDispatcher` and
...
:py`~arcade.gui.Property` is an pure-Python implementation of Kivy-like Properties.
