Source: doc/tutorials/platform_tutorial/step_03.rst
...
`arcade.SpriteList` exists to draw a collection of Sprites all at once. Let's say for example that
you have 100,000 box Sprites that you want to draw. You can add all of your boxes to a `arcade.SpriteList`
...
Quiz - where in our code should we place the data object of our `arcade.SpriteList`?
...
We're also doing a few new things in the `arcade.Sprite` creation. First off we are passing the image file
...
* Read the documentation for the `arcade.SpriteList` class
