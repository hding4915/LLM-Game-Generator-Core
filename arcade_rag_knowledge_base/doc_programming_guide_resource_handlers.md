Source: doc/programming_guide/resource_handlers.rst
...
The most common way is creating a :py`~arcade.Sprite` like this:
...
* Much of this page
* Arcade's tutorials <main-page-tutorials>
...
* does not warn against it like the resource_handles-adding section
  below

Most loading code in the areas below accepts a resource handle:

  * the example-code
  * the tutorials <main-page-tutorials>
...
If you're unsure, it's always okay to ask for help <how-to-get-help>.
...
The :py`arcade.resources.add_resource_handle` function allows you
...
```python
   ...
   import arcade
         ...
         * Ask someone else for help <how-to-get-help>
...
#. The following methods and properties on :py`pathlib.Path`: ``resolve()`` to get the absolute path, and
...
their own sprites inside. Since the :py`arcade.resources.resolve` function
returns a :py`~pathlib.Path` object, you can resolve the folder for a
character once, then write shorter code using :py`~pathlib.Path` slash
syntax:
...
2. If it does, return a :py`pathlib.Path` object for it
...
The handles are stored as a :py`dict` inside ``arcade.resources.handles``.

* Each resource handle's name is a :py`str` used to look up a :py`list`
  of :py`pathlib.Path` objects
* Each :py`~pathlib.Path` refers to a directory on disk and supports
